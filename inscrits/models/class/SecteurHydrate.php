...
class SecteurHydrate
{
	...
	public function add(Secteur $secteur){
		...
		if ($req->rowCount() > 0) {
			...
			try {
				...
				$req = $this->db->prepare("
					INSERT INTO tb_secteur (designation, icon) VALUES (?,?)
				");
				$dateadded = date('Y-m-d');
				$req->execute([ $secteur->designation(), $secteur->icon() ] );
	...
	{
		$q = $this->db->query('SELECT * FROM tb_secteur WHERE status = 1 ORDER BY designation');
		return $q;
	}
